...
rm -rf "${BUILD_DIR}"
mkdir -p     "${DEBIAN_DIR}"     "${BUILD_DIR}/usr/bin"     "${BUILD_DIR}/usr/lib/python3/dist-packages/intune_zabbix_bridge"     "${BUILD_DIR}/usr/lib/systemd/system"     "${BUILD_DIR}/usr/share/intune-zabbix-bridge/zabbix"     "${BUILD_DIR}/usr/share/doc/${PKG}"     "${BUILD_DIR}/etc/intune-zabbix-bridge"     "${DIST_DIR}"
