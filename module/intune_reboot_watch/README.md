...
0.7.15 fixes uptime aggregation so **Longest uptime** and the uptime threshold counters use every represented device with a usable recorded uptime, including stale telemetry rows. Telemetry freshness remains independent: stale telemetry still cannot prove weekly reboot compliance, but it no longer suppresses historical uptime from the headline statistic.

0.7.14 restored collection for summaries above 64,000 bytes using a versioned zlib/base64 envelope. The widget decodes every device row and counter, accepts old plain JSON history, and bounds decoding to 4,000,000 bytes. Upgrade the Debian package to update the collector and widget together; PHP zlib is required for compressed summaries.
...
0.7.13 fixed two dashboard semantics exposed by the restored full fleet. Column sorting ranks the complete fleet before the configured display limit is applied, so sorting Last restart, Telemetry collected, Uptime or any other column is genuinely global rather than being distorted by fault rows pinned into the visible set. Uptime colour is determined only by the displayed uptime value; stale telemetry remains independently orange in the Telemetry column.
