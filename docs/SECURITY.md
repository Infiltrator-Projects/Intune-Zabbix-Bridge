...
The bridge is read-only against Microsoft Intune. It reads managed-device inventory, Windows Update Ring configuration/device status, and device-health-script results. It does not connect to laptops, invoke remediation, restart devices, use WinRM or require LAN reachability.
...
The summary can contain device names, user principal names, update-ring names/status timestamps and reboot timestamps; normal Zabbix access controls should be used.
